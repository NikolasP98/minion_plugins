...
Location: `${CLAUDE_PLUGIN_ROOT}/skills/fork-sync/state/evaluation.json`
...
STATE_FILE="${CLAUDE_PLUGIN_ROOT}/skills/fork-sync/state/evaluation.json"
...
1. **Compute delta commits**:
   ...
   ```bash
   OLD_HEAD=$(jq -r '.snapshot.mirrorHead' ${CLAUDE_PLUGIN_ROOT}/skills/fork-sync/state/evaluation.json)
